...
if strcmp(data_mode, 'real')
    ...
    feat = double(data.train_X);
    label = double(data.noisy_y);
    label = double(label');
    gt_label = double(data.clean_y);
    gt_label = double(gt_label');
    trusted_index = data.trusted_idx + 1;
    X_trust = feat(trusted_index,:);
    trust_item_label = gt_label(trusted_index);
    % process trust item
