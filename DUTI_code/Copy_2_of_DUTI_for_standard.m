...
% load data
data = load('../data/game_trusted2_3class.mat');
...
feat = cat(1, test_feat, feat);
ids = cat(1, test_ids, ids);
label = cat(1, test_label, label);
X_trust = cat(1, X_trust, valid_feat);
trust_ids = cat(1, trust_ids, valid_ids);
trust_item_label = cat(1, trust_item_label, valid_gt_label);
% 
% feat = feat(1:100,:);
% ids = ids(1:100,:);
% label = label(1:100,:);
...
search_range = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45];
...
for search_iter=1:7
    ...
    num_bugs = sum(bugs);
    acc = sum(test_gt_label == y_debug(1:test_count)) / test_count;
...
fprintf('Best accuracy: best_c_value=%d\n', best_c_value);
fprintf('Test Accuracy: %d / %d = %f\n', sum(test_gt_label == best_y_debug(1:test_count)), test_count, best_acc);        
